...
class TripProvider with ChangeNotifier {
  ...
  Future<void> loadDestinations() async {
    ...
    try {
      final cachedData = await _tripRepository.getCachedDestinations();
    ...
    try {
      final remoteDestinations = await _tripRepository
          .fetchRemoteDestinations();
      if (remoteDestinations.isNotEmpty) {
        // 서버에서 가져온 최신 데이터로 교체
        _allDestinations = remoteDestinations;

        // 3. 최신 데이터를 다시 로컬 DB에 캐싱하여 다음 앱 실행 시 활용
        await _tripRepository.saveDestinations(remoteDestinations);
